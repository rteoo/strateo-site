...
- `assets/` holds the favicon, wordmark logos (SVG/PNG). Everything is flat at the repo root.
...
- **Asset cache-busting:** asset references use `?v=N` query strings (e.g. `assets/favicon.svg?v=3`). When you replace a file in `assets/`, bump the `?v=N` on every `<img>`/`<link>` pointing at it to bust browser/CDN caches.
